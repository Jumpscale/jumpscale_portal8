...
class WorkerSession(TCPSession):
    ...
    def run(self):
        ...
        while True:
            ...
            print("loopend")
            j.portal.active.messagerouter.queue(gid, nid, pid, data)
...
class ManholeSession(TCPSession):
    ...
    def __init__(self, addr, port, socket):
        ...
        self.type = "manhole"
        self.cmds = j.portal.active.tcpservercmds
...
"""
    ...
    def getsession(self, id):
        if id not in j.portal.active.sessions:
            self.send("Could not find session with id %s" % id)
            return False
        return j.portal.active.sessions[id]

    def killallsessions(self):
        result = ""
        for key in list(j.portal.active.sessions.keys()):
            session = j.portal.active.sessions[key]
